    ...
    private Object logTransaction(ProceedingJoinPoint jp, Transactional transactional) throws Throwable {
        ...
        if(TransactionSynchronizationManager.isActualTransactionActive()) {
            ...
            try {
                ...
                log.info("Transaction {} started by {} in {}.{}", id, user, callerClass, callerMethod);
            ...
            if (args != null) {
                log.info("Method {}.{} called by {} called in transaction {} with args: {}", callerClass, callerMethod, user, id, args);
            } else {
                log.info("Method {}.{} called by {} called in transaction {}", callerClass, callerMethod, user, id);
            }
            Object obj;
            try {
                obj = jp.proceed();
            } catch (Throwable e) {
                log.error("Method {}.{} called by {} failed in transaction {} due {} with message {}", callerClass, callerMethod, user, id, e.getClass().getName(), e.getMessage());
                throw e;
            }
            String returnValue = parseReturnValue(obj);
            if (returnValue != null) {
                log.info("Method {}.{} called by {} returned in transaction {} with: {}", callerClass, callerMethod, user, id, returnValue);
            } else {
                log.info("Method {}.{} called by {} returned in transaction {}", callerClass, callerMethod, user, id);
            }
            return obj;
        } else {
            ...
            String args = parsArgs(jp.getArgs());
            if (args != null) {
                log.info("Method {}.{} called by {} with args: {}", callerClass, callerMethod, user, args);
            } else {
                log.info("Method {}.{} called by {}", callerClass, callerMethod, user);
            }
            Object obj;
            try {
                obj = jp.proceed();
            } catch (Throwable e) {
                log.error("Method {}.{} called by {} failed due {} with message {}", callerClass, callerMethod, user, e.getClass().getName(), e.getMessage());
                throw e;
            }
            String returnValue = parseReturnValue(obj);
            if (returnValue != null) {
                log.info("Method {}.{} called by {} returned with: {}", callerClass, callerMethod, user, returnValue);
            } else {
                log.info("Method {}.{} called by {} returned", callerClass, callerMethod, user);
            }
            return obj;
        }
    }
